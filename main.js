...
var app = http.createServer(function(request, response){
  ...
  if (pathname === '/') {
    if (queryData.id === undefined) {
      ...
      db.query(`SELECT * FROM topic`, function(err, topics){
        if (err) throw err;
        db.query(`SELECT t.id, t.title, t.description, a.name FROM topic t LEFT JOIN author a ON t.author_id=a.id WHERE t.id=?`, [queryData.id], function(err2, topic){
          if (err2) throw err2;
              ...
              function(error3, authors) {
                ...
                html = template.HTML(
                  ...
                  `<form action="/update_process" method="post">
                    ...
                    <p><textarea name="description" palceholder="description">${topic[0].description}</textarea></p>
